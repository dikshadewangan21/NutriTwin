...
export default function Navbar({ activeTab, setActiveTab, userProfile, openChat, onLogout }) {
  ...
  return (
    <>
      <nav className="glass-panel sticky top-4 z-40 mx-4 my-3 px-6 py-3.5 flex items-center justify-between">
        {/* Brand */}
        <div
        ...
        >
          ...
          <div className="hidden sm:block">
            <h1 className="text-xl font-bold tracking-tight text-white">NutriTwin</h1>
            <p className="text-xs text-gray-400">Personalized Nutrition & Diet Platform</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={openChat}
            className="btn-primary py-2 px-3 text-xs rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 shadow-indigo-500/25"
          >
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 bg-gray-800/80 border border-gray-700 hover:border-emerald-500/50 px-3 py-2 rounded-xl text-xs transition"
            >
              {/* Avatar circle */}
              <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-emerald-500 to-indigo-500 flex items-center justify-center text-white font-bold text-sm">
                {userInitial}
              </div>
              <div className="hidden sm:block text-left">
                <p className="text-white font-semibold leading-none">{userName.split(' ')[0]}</p>
                <p className="text-[10px] text-emerald-400 mt-0.5">{userProfile?.assigned_cluster_label || 'Active'}</p>
              </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-30 flex">
          ...
          {/* Drawer */}
          <div className="w-72 bg-gray-950 border-l border-gray-800 flex flex-col p-6 space-y-2 overflow-y-auto">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-emerald-500 to-indigo-600 flex items-center justify-center text-xl">🥗</div>
              <span className="font-bold text-white text-lg">NutriTwin</span>
            </div>
            {navItems.map((item) => {
              const Icon = item.icon;
              const isActive = activeTab === item.id;
            ...
            <div className="pt-4 border-t border-gray-800 space-y-2">
              <button
                ...
                className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-sm text-gray-300 hover:bg-gray-800 transition"
              >
                <User className="w-4 h-4 text-emerald-400" />
                My Profile
              </button>
              <button
                ...
                className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-sm text-rose-400 hover:bg-rose-950/50 transition"
              >
                <LogOut className="w-4 h-4" />
                Sign Out
              </button>
